...
if (base64.length !== 48852 || raw.length !== 36638 || sha256 !== "77e9722375d29b1edba91d8b0340c25261c68f686773277c443ae9c1432d5f7c") {
  throw new Error(`静态图片数据错误：${base64.length}/${raw.length}/${sha256}`);
}
...
await page.goto("http://127.0.0.1:4173/?v=season1-real-13", { waitUntil: "networkidle" });
...
const card = await page.evaluate(async () => {
  ...
  return { width: image.naturalWidth, height: image.naturalHeight, marker: art.dataset.coverSource, background: getComputedStyle(art).backgroundImage };
});
...
const detail = await page.evaluate(() => {
  ...
  return { marker: visual.dataset.coverSource, background: getComputedStyle(visual).backgroundImage };
});

await page.screenshot({ path: "season1-real-v13.png", fullPage: true });
fs.writeFileSync("season1-real-v13.json", JSON.stringify({ sha256, card, detail, errors }, null, 2));
await browser.close();
