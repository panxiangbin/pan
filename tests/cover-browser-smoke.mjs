...
if (result.decodeResults.length !== 8) failures.push(`图片解码结果数量异常：${result.decodeResults.length}`);
...
if (consoleMessages.some(message => message.includes("ERR_INVALID_URL") || message.includes("Failed to load resource"))) {
  failures.push("浏览器仍报告封面资源加载错误");
}
