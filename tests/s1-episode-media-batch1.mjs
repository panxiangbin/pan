...
try {
  ...
  result.dialogTitle = await page.locator("#episodeStoryTitle").textContent();
  if (!result.dialogTitle?.includes("王室抵达临冬城")) throw new Error(`详情标题异常：${result.dialogTitle}`);
  stage = "complete";
} catch (error) {
...
result = {
  ...
  seasonButton: await page.locator(".season-mode-button").count(),
  seasonCards: await page.locator(".season-card").count(),
  episodeTabs: await page.locator('[data-story-view="episodes"]').count(),
  mediaCards: await page.locator(".season-episode-media").count(),
  bodyText: (await page.locator("body").innerText()).slice(0, 500)
};
fs.writeFileSync("s1-episode-media-batch1.json", JSON.stringify(result, null, 2));
await page.screenshot({ path: "s1-episode-media-batch1.png", fullPage: true });
await browser.close();
console.log(JSON.stringify(result, null, 2));
if (failure) throw new Error(`阶段 ${stage} 失败：${failure}`);
if (errors.some(error => error.includes("脚本加载失败"))) throw new Error(errors.join(" | "));
console.log(`第1季第1集图文剧情卡验证通过：${raw.length} bytes，${result.dimensions.width}×${result.dimensions.height}`);
