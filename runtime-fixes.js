(() => {
  ...
  function ensureStylesheet(href) {
    ...
    link.rel = "stylesheet";
    link.href = href;
    link.addEventListener("error", () => { link.dataset.failed = "true"; }, { once: true });
  ...
  ensureStylesheet("season1-episode-visuals.css?v=s1-episode-media-3");

  let episodeFeaturesPromise = null;
  function seasonButton() { return document.querySelector(".season-mode-button"); }
  ...
  function clearEpisodeLoadError() { document.querySelector(".episode-load-error")?.remove(); }
  function showEpisodeLoadError() {
    ...
    notice.innerHTML = `<span aria-hidden="true">!</span><div><strong>逐集剧情暂时没有加载成功</strong><small>章节导读仍可正常使用。可能是网络瞬断，点击重试即可继续加载73集内容。</small></div><button type="button">重新加载逐集剧情</button>`;
    const anchor = seasonOverlay.querySelector(".season-progress-card") || seasonOverlay.querySelector(".season-header");
    anchor?.insertAdjacentElement("beforebegin", notice);
    notice.querySelector("button")?.addEventListener("click", event => {
      event.currentTarget.disabled = true;
      event.currentTarget.textContent = "正在重新加载…";
  ...
  function loadEpisodeFeatures() {
    ...
    episodeFeaturesPromise = loadScript("episode-data.js?v=cover-art-10")
      ...
      .then(() => { clearEpisodeLoadError(); setEpisodeLoading(false); })
      .catch(error => { episodeFeaturesPromise = null; setEpisodeLoading(false); showEpisodeLoadError(); throw error; });
    return episodeFeaturesPromise;
  }

  const episodeVisuals = loadScript("season1-episode-media.js?v=s1-episode-media-3")
    .then(() => loadScript("season1-episode-visuals.js?v=s1-episode-media-3"));
  ...
  const seasonFeatures = loadScript("season-cover-sprite.js?v=cover-art-10")
    ...
    .then(() => loadScript("season1-real-cover.js?v=season1-real-13"))
    .then(() => loadScript("season-infographics.js?v=cover-art-10"))
    .then(() => {
      seasonButton()?.addEventListener("click", () => loadEpisodeFeatures().catch(error => console.error("逐集剧情模块加载失败：", error)), { once: true });
      if (/^#season-[1-8]-episode-\d{1,2}$/.test(location.hash)) return loadEpisodeFeatures();
      const preload = () => loadEpisodeFeatures().catch(error => console.error("逐集剧情模块预加载失败：", error));
      if ("requestIdleCallback" in window) window.requestIdleCallback(preload, { timeout: 5000 });
      else window.setTimeout(preload, 3200);
      return null;
    });

  Promise.all([episodeVisuals, portraitFeatures, seasonFeatures]).catch(error => console.error("扩展模块加载失败：", error));
})();